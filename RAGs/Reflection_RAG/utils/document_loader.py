...
def load_documents(data_dir: Path) -> List[str]:
    ...
    Returns a list of (content, source_path) as flat text per file for chunking.
    """
...
def chunk_text(
    ...
    Uses simple recursive split on newlines then spaces to approximate chunk_size.
    """
    if overlap >= chunk_size:
        overlap = chunk_size // 2
    chunks: List[str] = []
    for doc in documents:
        doc_chunks = _split_into_chunks(doc, chunk_size, overlap)
